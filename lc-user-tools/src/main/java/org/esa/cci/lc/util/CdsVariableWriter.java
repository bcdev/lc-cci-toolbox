...
public class CdsVariableWriter extends ProfilePartIO {
    ...
    private void lcLatLonCustomBoundsWriter(ProfileWriteContext ctx, Product p, Double minLon, Double maxLon, Double minLat, Double maxLat) throws IOException {
        Array data;
        final NFileWriteable writeable = ctx.getNetcdfFileWriteable();
        int numLatPixels = p.getSceneRasterHeight();
