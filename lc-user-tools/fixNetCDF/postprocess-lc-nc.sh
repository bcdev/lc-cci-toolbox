...
if [ "$1" = "" ]; then
    echo "Usage: $0 ESACCI\*${oldVersion}\*nc"
    echo "Generates files ESACCI\*${newVersion}\*nc"
    echo "Edit version numbers before application."
    exit 1
fi
...
while [ "$1" != "" ]; do
    input=$1
    shift
    output=${input/${oldVersion}/${newVersion}}
    tempOut0=${input%.nc}-temp0.nc
    tempOut=${input%.nc}-temp.nc
    if [ "$input" = "$output" ]; then
        echo "version $oldVersion not found in $input"
        continue
    fi
...
ncap2 -h -O -4 -L 6 -s 'temp_lccs_class=lccs_class.ubyte()' $input $tempOut0
...
ncks -h -O -4 -L 6 -a -x -v lccs_class --cnk_dmn lat,$chunkSize --cnk_dmn lon,$chunkSize $tempOut0 $tempOut
